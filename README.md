...
```javascript
// ==UserScript==
// @name         ChatGPT & SheerID 自动化验证循环助手 (精准版)
// @namespace    http://tampermonkey.net/
// @version      2.0
// @description  精准定位按钮与错误标题，自动化点击与重试循环
// @author       Gemini
...
(function() {
    ...
    // 场景二：在 SheerID 页面，检测错误并回退
    else if (window.location.host.includes(CONFIG.errorUrlHost)) {
        console.log("脚本：处于 SheerID 页面，正在监控状态...");
